...
const NAV: { key: Page; label: string; path: string }[] = [
  {
    key: 'dashboard',
    label: 'Dashboard',
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-30 w-[220px] flex flex-col bg-surface-card border-r border-border">
      ...
      <div className="flex items-center gap-3 px-5 h-16 border-b border-border shrink-0">
        <div className={cn('w-8 h-8 rounded-xl flex items-center justify-center', theme === 'aurora' ? 'aurora-brand' : 'bg-accent/10')}>
          <svg className="w-4 h-4 text-accent" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
          </svg>
        </div>
        <div>
          <p className="text-sm font-bold text-foreground leading-tight">CryptoBot</p>
          <p className="text-xs text-muted">Trading Engine</p>
        </div>
      </div>
