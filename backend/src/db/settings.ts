...
export function getSettings(): BotSettings {
  ...
  return {
    ...
    signal_model: (map.signal_model === 'agent' ? 'agent' : 'classic') as 'classic' | 'agent',
    offline_mode_forced: map.offline_mode_forced === 'true',
    offline_auto: (map.offline_auto ?? 'true') === 'true',
    offline_reuse_max_age_min: parseInt(map.offline_reuse_max_age_min || '30', 10),
    agent_signal_check_portfolio: (map.agent_signal_check_portfolio ?? 'true') === 'true',
    agent_signal_retain_runs: parseInt(map.agent_signal_retain_runs || '200', 10),
    ...
    binance_read_only: (map.binance_read_only ?? 'true') === 'true',
    ...
    // Renamed from monitor_d_sequential / monitor_d_retain_runs — fall back to the old key so
    // a pre-rename DB carries the user's value over without a write migration.
    monitor_sequential: (map.monitor_sequential ?? map.monitor_d_sequential ?? 'true') === 'true',
    monitor_retain_runs: parseInt(map.monitor_retain_runs || map.monitor_d_retain_runs || '200', 10),
    ...
    entry_max_pullback_pct: parseFloat(map.entry_max_pullback_pct || '1'),
    ...
    entry_agent_candle_count: parseInt(map.entry_agent_candle_count || '24', 10),
    entry_agent_retain_runs: parseInt(map.entry_agent_retain_runs || '200', 10),
    coach_auto_run: map.coach_auto_run === 'true',
    coach_cron: map.coach_cron || '0 6 * * *',
    coach_min_trades: parseInt(map.coach_min_trades || '15', 10),
    coach_retain_runs: parseInt(map.coach_retain_runs || '100', 10),
    ...
    llm_entryAgent_endpoint: map.llm_entryAgent_endpoint || '',
    llm_entryAgent_max_tokens: parseInt(map.llm_entryAgent_max_tokens || '0', 10),
    llm_agent_endpoint: map.llm_agent_endpoint || '',
    llm_agent_max_tokens: parseInt(map.llm_agent_max_tokens || '0', 10),
    // Renamed from llm_monitor_d_* — fall back to the old key for a pre-rename DB.
    llm_monitor_endpoint: map.llm_monitor_endpoint || map.llm_monitor_d_endpoint || '',
    llm_monitor_max_tokens: parseInt(map.llm_monitor_max_tokens || map.llm_monitor_d_max_tokens || '0', 10),
    llm_agentSignal_endpoint: map.llm_agentSignal_endpoint || '',
    llm_agentSignal_max_tokens: parseInt(map.llm_agentSignal_max_tokens || '0', 10),
    llm_coach_endpoint: map.llm_coach_endpoint || '',
    llm_coach_max_tokens: parseInt(map.llm_coach_max_tokens || '0', 10),
    llm_webSearch_endpoint: map.llm_webSearch_endpoint || '',
    llm_webSearch_max_tokens: parseInt(map.llm_webSearch_max_tokens || '0', 10),
    // Per-module streaming, default ON (only an explicit 'false' disables it).
    llm_stream_analyst: map.llm_stream_analyst !== 'false',
    llm_stream_extractor: map.llm_stream_extractor !== 'false',
    llm_stream_discoverer: map.llm_stream_discoverer !== 'false',
    llm_stream_discovererExtractor: map.llm_stream_discovererExtractor !== 'false',
    llm_stream_summary: map.llm_stream_summary !== 'false',
    llm_stream_entryAgent: map.llm_stream_entryAgent !== 'false',
    llm_stream_agent: map.llm_stream_agent !== 'false',
    llm_stream_monitor: (map.llm_stream_monitor ?? map.llm_stream_monitorD) !== 'false',
    llm_stream_agentSignal: map.llm_stream_agentSignal !== 'false',
    llm_stream_coach: map.llm_stream_coach !== 'false',
    llm_stream_webSearch: map.llm_stream_webSearch !== 'false',
    ...
    llm_entryAgent_fb_endpoint: map.llm_entryAgent_fb_endpoint || '',
    llm_entryAgent_fb_max_tokens: parseInt(map.llm_entryAgent_fb_max_tokens || '0', 10),
    llm_agent_fb_endpoint: map.llm_agent_fb_endpoint || '',
    llm_agent_fb_max_tokens: parseInt(map.llm_agent_fb_max_tokens || '0', 10),
    llm_monitor_fb_endpoint: map.llm_monitor_fb_endpoint || map.llm_monitor_d_fb_endpoint || '',
    llm_monitor_fb_max_tokens: parseInt(map.llm_monitor_fb_max_tokens || map.llm_monitor_d_fb_max_tokens || '0', 10),
    llm_agentSignal_fb_endpoint: map.llm_agentSignal_fb_endpoint || '',
    llm_agentSignal_fb_max_tokens: parseInt(map.llm_agentSignal_fb_max_tokens || '0', 10),
    llm_coach_fb_endpoint: map.llm_coach_fb_endpoint || '',
    llm_coach_fb_max_tokens: parseInt(map.llm_coach_fb_max_tokens || '0', 10),
    llm_webSearch_fb_endpoint: map.llm_webSearch_fb_endpoint || '',
    llm_webSearch_fb_max_tokens: parseInt(map.llm_webSearch_fb_max_tokens || '0', 10),
    ...
    chart_candle_limit: parseInt(map.chart_candle_limit || '150', 10),
    chart_marker_limit: parseInt(map.chart_marker_limit || '200', 10),
...
// BotSettings shape (e.g. the routing graph or debug_log_retain). Served from the same cache.
